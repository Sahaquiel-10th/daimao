...
export default function BillingPage({ onError, onToast }) {
  ...
  async function load(nextPage = page, nextId = selectedId) {
    setLoading(true);
    try {
      const payload = await callAdmin("adminGetAppClientBilling", {
  ...
  return <section className="content-grid billing-page">
      ...
      <table className="billing-clients"><thead><tr><th>社区 / 客户端</th><th>AppID</th><th>状态</th><th>余额</th><th>预警线</th><th>计费方式</th><th>操作</th></tr></thead>
        <tbody>{clients.length ? clients.map((client) => {
          ...
          return <tr key={id} className={String(id) === String(selectedId) ? "selected-row" : ""} onClick={() => setSelectedId(String(id))}>
            <td><div className="title-stack"><strong>{clientName(client)}</strong><span>{communityOf(client)}</span></div></td>
    ...
    <div className="billing-detail-grid">
      <BillingTable title="AI 用量明细" rows={result?.usageEvents || []} type="usage" />
      <BillingTable title="电力账本" rows={result?.walletLedger || []} type="ledger" />
    </div>
    <Pagination pagination={result?.pagination} page={page} onPage={(next) => { setPage(next); load(next); }} />
...
function Pagination({ pagination = {}, page, onPage }) { const total = number(pick(pagination, "totalPages", "total_pages")) || 1; return <div className="billing-pagination"><button disabled={page <= 1} onClick={() => onPage(page - 1)}>上一页</button><span>第 {page} / {total} 页</span><button disabled={page >= total} onClick={() => onPage(page + 1)}>下一页</button></div>; }
...
function BillingModal({ state, platform, loading, onClose, onSubmit }) {
  ...
  const [form, setForm] = useState(() => {
    ...
    if (state.type === "client") return { appid: "", name: "", companyName: "", communityId: "", clientType: "wechat_miniprogram", status: "active", allowedActions: "runAiTask startSecretaryChat" };
  ...
  function submit(e) {
    ...
    if (state.type === "client") return onSubmit("adminUpsertAppClient", { client: { ...form, communityId: form.communityId ? number(form.communityId) : null, allowedActions: form.allowedActions.split(/[\s,，]+/).filter(Boolean) } }, "App Client 已创建");
  ...
  const title = state.type === "adjust" ? `${form.mode === "add" ? "增加" : "扣减"}电力 · ${clientName(client)}` : state.type === "settings" ? `计费设置 · ${clientName(client)}` : state.type === "platform" ? "平台统一计费设置" : state.type === "client" ? "新建 App Client" : state.type === "token" ? `计费只读令牌 · ${clientName(client)}` : `钱包状态 · ${clientName(client)}`;
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal-card dm-card billing-modal" onMouseDown={(e) => e.stopPropagation()}><div className="modal-header"><h3>{title}</h3><button type="button" onClick={onClose}>关闭</button></div><form className="billing-form" onSubmit={submit}>
    ...
    {state.type === "client" && <>{[["appid","AppID"],["name","小程序名称"],["companyName","公司名称"],["communityId","社区 ID"]].map(([key,label]) => <label key={key}>{label}<input required={key === "appid" || key === "name"} value={form[key]} onChange={(e) => set(key,e.target.value)} /></label>)}<label>客户端类型<select value={form.clientType} onChange={(e) => set("clientType",e.target.value)}><option value="wechat_miniprogram">微信小程序</option><option value="web">Web</option><option value="server">服务端</option></select></label><label>允许的 actions<textarea value={form.allowedActions} onChange={(e) => set("allowedActions",e.target.value)} /></label></>}
